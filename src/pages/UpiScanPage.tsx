...
import { analyzeUpi, calculateUpiRiskScore, generateUpiExplanation, generateUpiRecommendations } from '@/lib/upiScoring';
...
export default function UpiScanPage() {
  ...
  const handleScan = async () => {
    ...
    const analysis = analyzeUpi({ message: message.trim() });
    const { score: ruleScore, confidence: ruleConfidence, breakdown } = calculateUpiRiskScore(analysis);

    let ai: AIAnalysisResult | null = null;
    try {
      ai = await runAIAnalysis('upi', message.trim());
    } catch {
    ...
    const explanation = generateUpiExplanation(analysis, hybrid.finalScore, hybrid.finalConfidence);
    if (ai?.explanation) explanation.summary = ai.explanation;
    if (ai?.fraudType) explanation.malwareCategory = ai.fraudType;

    const recommendations = generateUpiRecommendations(hybrid.finalScore, riskLevel);
    ...
    const scanResult: FraudScanResult = {
      ...
      riskBreakdown: { metadata: breakdown.collectRequest, behavior: breakdown.bankVerification, urlStructure: breakdown.merchantLink, patternMatch: breakdown.qrCode },
      explanation,
      recommendations,
  ...
  return (
    <AppLayout>
      <div className="space-y-6 fade-in">
        <div>
          <h1 className="text-3xl font-display font-bold">UPI Fraud Analyzer</h1>
          <p className="text-muted-foreground mt-1">AI-powered UPI payment fraud detection</p>
        </div>

        <DemoMode onSelect={handleDemo} filterType="upi" />
        ...
        <Card className="p-6 bg-card border-border">
          ...
          <div className="space-y-4">
            <Textarea placeholder="Paste the suspicious UPI/payment message here..." value={message} onChange={(e) => setMessage(e.target.value)} rows={5} className="bg-secondary border-border" />
        ...
        {result && (
          <div className="space-y-6 fade-in">
            ...
            <FeedbackButtons scanId={result.id} scanType="upi" content={message} originalScore={result.riskScore} />
          </div>
        )}
      </div>
    </AppLayout>
